# Running MaxioPassthroughApiTests through the built Release DLL

`MaxioPassthroughApiTests` can be run either the normal way (`dotnet test` from source) or by
...
Assumes `MaxioPassthroughApiTests/build/` already has a Release build (`.\build.ps1`) and that
the mock and target PublicApi (Direct or Plugin) are already running per root `CLAUDE.md`'s
end-to-end recipe.

From `MaxioPassthroughApiTests/build/`, using `dotnet vstest` (not `dotnet test` — there's no
project/solution here, just the built assembly):

**PowerShell:**
```powershell
cd D:\work\eshop-integration\eshopOnWeb\MaxioPassthroughApiTests\build
$env:PUBLICAPI_BASEURL = "http://localhost:5199"
dotnet vstest MaxioPassthroughApiTests.dll --logger:"junit;LogFilePath=../../docs/test-logs.xml"
```

**Bash:**
```bash
cd "D:/work/eshop-integration/eshopOnWeb/MaxioPassthroughApiTests/build"
PUBLICAPI_BASEURL=http://localhost:5199 dotnet vstest MaxioPassthroughApiTests.dll \
...
- `LogFilePath` is resolved relative to the working directory you run the command from, so the
  path above (`../../docs/test-logs.xml`) only works when run from `MaxioPassthroughApiTests/build`.
