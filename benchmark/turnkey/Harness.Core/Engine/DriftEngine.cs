...
/// an added field, a scalar that became a union, or a renamed envelope key. The integration's code is
/// never touched: we replay an already-produced integration against a drifted provider and observe
/// whether it still returns correct data, degrades gracefully, breaks, or silently corrupts.
///
/// Installed rules apply while present and are cleared by POST /__mock/reset. With no rules installed
/// the middleware is a no-op, so the mock behaves byte-identically when drift is not being exercised.
///
/// PROVIDER-AGNOSTIC: the transforms operate on arbitrary JSON; the field/envelope names come from the
/// per-op drift plan in the op table, not from any hard-coded provider knowledge.
/// </summary>
    ...
    public static string Apply(DriftRule rule, string json)
    {
        ...
        switch (rule.Profile.ToLowerInvariant())
        {
            case "additive":                                   // P1: add unknown scalar + nested object
                ...
                break;
            case "rename":                                     // P2: rename a leaf the integration reads
