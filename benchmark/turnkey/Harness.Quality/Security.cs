...
///  • Supply-chain surface — transitive dependency count + `dotnet list --vulnerable`, counted on the
///    project that carries the provider client + any SDK package (profile.analysis.depProjectFile).
///  • Source scan — regex sweep of the integration files for hardcoded secrets, non-TLS base URLs baked
///    into code, and disabled TLS/cert validation.
/// This is the .NET adapter; other stacks swap the dependency command + keep the source rules.
/// </summary>
public static class Security
{
    ...
    public static D4Report Analyze(string treeRoot, string pathPattern, string depProjectFile)
    {
        var infra = FindProject(treeRoot, string.IsNullOrEmpty(depProjectFile) ? "Infrastructure.csproj" : depProjectFile)
                    ?? FindProject(treeRoot, "*.csproj");
        ...
        foreach (var f in Metrics.IntegrationFiles(treeRoot, pathPattern))
        {
