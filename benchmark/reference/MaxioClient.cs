...
public sealed record Breaks(bool Leak, bool RetryWrite, bool NoTimeout, bool Raw500, bool NoAuth, bool LogSecret)
{
    public static Breaks From(string? env)
    {
        var s = (env ?? "").ToLowerInvariant();
        bool H(string k) => s.Contains(k);
        return new Breaks(H("leak"), H("retrywrite"), H("notimeout"), H("raw500"), H("noauth"), H("logsecret"));
    }
}
