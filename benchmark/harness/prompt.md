...
Expose the following **11 HTTP endpoints under `/api/billing`** in the **PublicApi** host, each backed by
...
- A failed **write** (create subscription, plan change, record usage) must **never be duplicated** by a retry.
