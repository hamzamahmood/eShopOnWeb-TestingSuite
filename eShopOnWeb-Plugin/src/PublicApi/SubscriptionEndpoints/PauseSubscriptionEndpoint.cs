...
/// <summary>UC4: pauses (holds) an active subscription.</summary>
public class PauseSubscriptionEndpoint : IEndpoint<IResult, LifecycleRequest, SubscriptionEndpointContext>
{
    ...
    public async Task<IResult> HandleAsync(LifecycleRequest request, SubscriptionEndpointContext context)
    {
        try
        {
            var subscription = await context.SubscriptionService.PauseAsync(request.SubscriptionId, context.CallerUserId, context.CallerIsAdmin, context.CancellationToken);
