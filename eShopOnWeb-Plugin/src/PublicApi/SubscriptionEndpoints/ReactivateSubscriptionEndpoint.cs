...
/// <summary>UC4: reactivates a previously canceled subscription.</summary>
public class ReactivateSubscriptionEndpoint : IEndpoint<IResult, LifecycleRequest, SubscriptionEndpointContext>
{
    ...
    public async Task<IResult> HandleAsync(LifecycleRequest request, SubscriptionEndpointContext context)
    {
        try
        {
            var subscription = await context.SubscriptionService.ReactivateAsync(request.SubscriptionId, context.CallerUserId, context.CallerIsAdmin, context.CancellationToken);
