...
/// <summary>
/// Exercises the read-only customer-lookup endpoint (<c>GET /api/maxio/customers/lookup?reference=…</c>).
/// </summary>
...
public class CustomerLookupTests : BlackBoxTest
{
    ...
    {
        const string intent = "Look up a customer by a known reference";
    ...
    {
        const string intent = "Look up a customer by an unknown reference";
        using var client = new ApiClient();
